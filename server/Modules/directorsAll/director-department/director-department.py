...
username = "skhotwow"
password = "555552026Zz10101010"
L.login(username, password)


def InsertDataSocials(social_network, channel, followers):
    date = now.strftime('%Y-%m-%d %H:%M:%S')
    print(social_network)
    insert_socials_query = ("INSERT INTO socials_network"
                            "(social_network, date, channel, followers)"
                            "VALUES (%s, %s, %s, %s)")
    data_socials = (
        social_network, date, channel, followers)
    cursor.execute(insert_socials_query, data_socials)
    cnx.commit()


def GrabFollowersInstagram():
    profile = instaloader.Profile.from_username(L.context, "polina_education")
    InsertDataSocials(followers=profile.followers, social_network="instagram", channel="polina_education")


def GrabFollowersTelegram():
    pass


schedule.every(24).hours.do(GrabFollowersInstagram)
# schedule.every(10).seconds.do(GrabFollowersInstagram)
while True:
...
def addDeals():
    ...
    cost_money = request.args.get('cost_money')
    left_cost_money = request.args.get('left_cost_money')
    payed_money = request.args.get('payed_money')
...
def updateDeals():
    ...
    if success:
        ...
        for i in range(len(items)):
            ...
            deal_cost = json_users_data["info"]["items"][i][10]
            deal_paid = json_users_data["info"]["items"][i][11]
            deal_commission = json_users_data["info"]["items"][i][12]
            deal_received = json_users_data["info"]["items"][i][13]
            deal_earned = json_users_data["info"]["items"][i][17]
